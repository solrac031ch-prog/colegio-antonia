...
const ENGLISH_APP_VERSION = '12.0.0';
...
function assertRequiredElements() {
  ...
  if (missing.length) throw new Error(`Faltan elementos de Inglés: ${missing.join(', ')}`);
}
...
function saveEnglishProgress() {
  try {
    ...
    // La práctica continúa aunque el almacenamiento esté bloqueado.
  }
...
const questionBanks = {
  vocabulary: [
    ...
    q('¿Qué significa esta palabra?', 'hungry', 'tener hambre', ['tener hambre', 'estar cansado/a', 'estar feliz', 'correr rápido'], 'Hungry significa tener hambre.'),
    q('¿Qué significa esta palabra?', 'playground', 'patio de juegos', ['patio de juegos', 'dormitorio', 'baño', 'comedor'], 'Playground es un lugar para jugar.'),
  ...
  grammar: [
    q('Completa la oración.', 'She ___ like apples.', "doesn't", ["doesn't", "don't", 'is', 'do'], 'Con she usamos doesn’t: She doesn’t like apples.'),
    ...
    q('Completa la oración.', 'He ___ a blue backpack.', 'has', ['has', 'have', 'is', 'do'], 'Con he usamos has.'),
    q('Completa la oración.', 'They ___ two dogs.', 'have', ['have', 'has', 'does', 'are'], 'Con they usamos have.'),
    q('Completa la oración.', 'She ___ happy.', 'is', ['is', 'are', 'am', 'do'], 'Con she usamos is.'),
    q('Completa la oración.', 'We ___ at school.', 'are', ['are', 'is', 'am', 'does'], 'Con we usamos are.'),
    q('Completa la oración.', 'I ___ swim.', 'can', ['can', 'does', 'has', 'am'], 'Can expresa habilidad: I can swim.'),
    q('Elige la pregunta correcta.', 'Preguntar si ella tiene una mascota', 'Does she have a pet?', ['Does she have a pet?', 'Do she has a pet?', 'She does have a pet?', 'Does she has a pet?'], 'Después de does usamos have.'),
  ...
  reading: [
    ...
    q('Lee y responde: ¿Cómo está el clima?', 'It is cold and rainy today. Emma wears her coat and takes an umbrella.', 'cold and rainy', ['cold and rainy', 'hot and sunny', 'warm and windy', 'snowy and hot'], 'El texto dice: It is cold and rainy today.', true),
    q('Lee y responde: ¿Qué hace Leo primero después de la escuela?', 'After school, Leo does his homework. Then he rides his bike in the park.', 'he does his homework', ['he does his homework', 'he rides his bike', 'he goes to bed', 'he eats breakfast'], 'Primero Leo hace su tarea.', true),
    q('Lee y responde: ¿Qué color es la bicicleta?', 'Sara has a purple bike and a red helmet. She rides her bike with her dad on Sundays.', 'purple', ['purple', 'red', 'blue', 'green'], 'El texto dice: Sara has a purple bike.', true),
    q('Lee y responde: ¿Dónde almuerza Max?', 'Max goes to school at eight. At lunchtime, he eats in the school cafeteria with his friends.', 'in the school cafeteria', ['in the school cafeteria', 'at home', 'in the park', 'at the library'], 'El texto dice que come in the school cafeteria.', true),
  ],
  conversation: [
    q('Elige la mejor respuesta.', 'Do you like pizza?', 'Yes, I do.', ['Yes, I do.', 'Yes, I am.', 'Yes, she does.', 'No, I can.'], 'A Do you…? podemos responder Yes, I do.'),
    ...
    q('Elige la mejor respuesta.', 'How old are you?', "I'm nine.", ["I'm nine.", 'I have nine.', 'It is nine.', 'Yes, I am.'], 'Para decir la edad usamos I’m nine.'),
    ...
    q('Elige la mejor respuesta.', 'What is your favourite colour?', 'Purple.', ['Purple.', 'Nine years old.', 'At school.', 'Yes, I do.'], 'La pregunta pide un color.'),
    ...
    q('Elige la mejor respuesta.', 'What do you do after school?', 'I do my homework.', ['I do my homework.', 'I am homework.', 'She does school.', 'Yes, I do.'], 'La pregunta pide una actividad.'),
  ],
  writing: [
    q('Elige la oración bien escrita.', 'ella tiene un perro', 'She has a dog.', ['She has a dog.', 'She have a dog.', 'Has she a dog.', 'She dog has.'], 'Con she usamos has.'),
    ...
    q('Elige la oración bien escrita.', '¿Te gusta el helado?', 'Do you like ice cream?', ['Do you like ice cream?', 'Does you like ice cream?', 'You do like ice cream?', 'Do you likes ice cream?'], 'Con you usamos Do y luego like.'),
    ...
    q('Elige la oración bien escrita.', 'Ella puede bailar.', 'She can dance.', ['She can dance.', 'She can dances.', 'She does can dance.', 'Can she dances.'], 'Después de can usamos dance, sin s.'),
    ...
    q('Elige la oración bien escrita.', '¿Ella tiene una hermana?', 'Does she have a sister?', ['Does she have a sister?', 'Does she has a sister?', 'Do she have a sister?', 'She does have sister?'], 'Después de does usamos have.'),
    q('Elige la oración bien escrita.', 'Estoy en la escuela.', 'I am at school.', ['I am at school.', 'I is at school.', 'I are school.', 'Am I at school.'], 'Con I usamos am.'),
    q('Elige la oración bien escrita.', 'El gato está debajo de la mesa.', 'The cat is under the table.', ['The cat is under the table.', 'The cat under is table.', 'The cat are under table.', 'Under the cat is table.'], 'Para ubicación usamos is under.'),
  ],
};
...
function validateEnglishQuestion(question) {
  const correct = String(question.correct);
  const options = [];
  [correct, ...question.options.map(String)].forEach(option => {
    if (!options.includes(option)) options.push(option);
  });
  if (!question.prompt || !question.display || !correct || options.length < 2) throw new Error('Pregunta de Inglés inválida');
  return {
    ...
    reading: Boolean(question.reading),
  };
}

function makeSessionQuestions(topic) {
  const bank = shuffle(bankFor(topic));
  if (!bank.length) throw new Error(`No hay preguntas para ${topic}`);
  const questions = [];
  for (let index = 0; index < ENGLISH_SESSION_LENGTH; index += 1) {
    questions.push(validateEnglishQuestion(bank[index % bank.length]));
  }
  return questions;
}

function startEnglishSession(topic) {
  englishState.topic = englishTopics.some(item => item.key === topic) ? topic : 'vocabulary';
  ...
  englishState.questions = makeSessionQuestions(englishState.topic);
...
function getCurrentEnglishQuestion() {
  return englishState.questions[englishState.questionIndex] || null;
}

function isCurrentEnglishQuestion(questionId) {
  const current = getCurrentEnglishQuestion();
  return Boolean(current && current.id === questionId && englishState.currentQuestionId === questionId);
}
...
function renderEnglishQuestion() {
  ...
  current.options.forEach(option => {
    ...
    button.addEventListener('click', () => answerEnglish(current.id, option, button));
...
function finishEnglishAnswer(questionId) {
  if (!isCurrentEnglishQuestion(questionId)) return;
  const current = getCurrentEnglishQuestion();
  ...
  saveEnglishProgress();
  els.streak.textContent = String(englishState.streak);
  els.next.textContent = englishState.questionIndex === ENGLISH_SESSION_LENGTH - 1 ? 'Ver resultado 🌟' : 'Next →';
  els.next.classList.remove('hidden');
}

function answerEnglish(questionId, value, button) {
  if (englishState.processing || englishState.answered || button.disabled) return;
  if (!isCurrentEnglishQuestion(questionId) || button.dataset.questionId !== questionId) return;
  ...
  try {
    ...
    if (String(value) === current.correct) {
      ...
      if (englishState.attempts === 1) feedback('success', '🌟 ¡Muy bien!', 'Lo miraste y lo corregiste.', current.correct);
      else feedback('success', englishState.streak >= 3 ? '🔥 Great streak!' : '✨ Great job!', current.explanation, current.correct);
      finishEnglishAnswer(questionId);
      return;
    }
...
function nextEnglishQuestion() {
  if (!englishState.answered || englishState.processing) return;
  englishState.answered = false;
  englishState.currentQuestionId = null;
  if (englishState.questionIndex < ENGLISH_SESSION_LENGTH - 1) {
...
function finishEnglishSession() {
  ...
  englishProgress.byTopic[englishState.topic] = (Number(englishProgress.byTopic[englishState.topic]) || 0) + 1;
  saveEnglishProgress();
...
function registerServiceWorker() {
  ...
  window.addEventListener('load', async () => {
    try {
      ...
      // Inglés funciona online aunque el service worker falle.
    }
  });
}
...
function initEnglish() {
  ...
  document.documentElement.dataset.appVersion = ENGLISH_APP_VERSION;
}

try {
  initEnglish();
} catch (error) {
  console.error('No se pudo iniciar Inglés', error);
  const main = document.querySelector('main');
  if (main) {
    const notice = document.createElement('div');
    notice.className = 'panel';
    notice.setAttribute('role', 'alert');
    notice.innerHTML = '<h2>Necesitamos recargar</h2><p>Inglés no pudo iniciar correctamente. Cierra esta ventana y vuelve a abrirla.</p>';
    main.prepend(notice);
  }
}
